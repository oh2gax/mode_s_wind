...
- **Approach History Hi/Lo column toggle** — a new **Lo/Hi** button in the Approach History panel header switches between two views of the same server-side data: *Lo* shows the original 5 columns (1 000 / 1 500 / 2 000 / 2 500 / 3 000 ft) for a compact overview; *Hi* shows all 15 columns (200–3 000 ft at 200 ft steps) and the panel expands to `width: max-content` (`.ws-aphist-hi` CSS modifier class) so all columns are visible without horizontal scrolling, using the map area beneath; the table `<thead>` is now dynamically generated by JS to match the active band selection, and band cells in body rows are also rendered dynamically so no static column count is hardcoded in the HTML
