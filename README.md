...
| **ADS-B** | Teal | The aircraft is transmitting ADS-B extended squitters right now (any DF17 message within 30 s — identification, velocity, status …) and its position is known (MLAT), but it has sent no ADS-B position of its own for ≥ 45 seconds during this visit — counted from its last own position, or from the start of the visit if none has arrived yet | ADS-B GPS position dropout covered by MLAT — the aircraft's own GPS has failed but the Radarcape multilateration network continues to track it; the most operationally relevant jamming signal at EFHK where MLAT coverage is strong. Also catches aircraft that are already jammed when they come into range |

The **ADS-B** signal uses three per-visit fields maintained in `live_state`: `last_adsb_pos_ts` (updated only when a genuine TC=9-18/20-22 position message is decoded from the Beast feed — not from BDS 5,0/6,0 replies or cached state), `last_es_ts` (any DF17 extended squitter) and `first_seen` (start of the visit; an aircraft is re-created after 10 minutes of silence). Aircraft that stop all extended squitters are not flagged (that is not a GPS symptom), and nothing is inherited from an earlier visit of the same aircraft. The Gap signal requires `lat is None`, which almost never occurs at EFHK since MLAT immediately fills in positions when ADS-B GPS fails. ADS-B and Gap are mutually exclusive by design.

**Counting-method versions:** every hourly row stores the counting method it was recorded with (`method` column), and the charts mark each change with a dashed amber line (`v2`, `v3`); values on either side of a marker are not directly comparable.
...
| 3 | 2026-09-26 (deployment) | ADS-B loss counted per visit as described above (requires current extended-squitter activity; also flags aircraft jammed on entry); current hour survives restarts |
...
| method | Counting-method version the hour was recorded with (1 / 2 / 3, see *Counting-method versions* under GPS Quality); existing rows were assigned 1 or 2 automatically by timestamp |
