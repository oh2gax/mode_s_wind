...
**Val (value labels):** the `Val` segment at the right end of the `Barbs · Hi · Auto · Val` group shows or hides the `dir/spd` label next to each barb. With Val off and no component mode active, only the barbs are drawn; with HW or XW active, only the component value (e.g. `+12kt`) is shown, placed just above the barb. Dcl is greyed out while Val is off since there is only one label to place. The corner label shows `· NOVAL` when values are hidden. The setting is remembered across page loads (`localStorage`) and is kept, greyed out, while Barbs is off.
...
In **HW mode** (amber button) the headwind/tailwind component is shown at each barb: `+15kt` = headwind, `−8kt` = tailwind, colour-coded **green** (headwind > 5 kt), **red** (tailwind < −5 kt), **amber** (near-zero). The component is computed as `wind_spd × cos(wind_dir − runway_heading)`.

In **XW mode** (orange button) the crosswind component is shown using the same `+`/`−` sign format as HW: `+8kt` = wind from the right of the centreline, `−3kt` = from the left. Colour-coded by magnitude — **teal** (< 5 kt, light), **amber** (5–9 kt, moderate), **red** (≥ 10 kt, strong) — matching the Approach History XW palette. Computed as `wind_spd × sin(wind_dir − runway_heading)`.
